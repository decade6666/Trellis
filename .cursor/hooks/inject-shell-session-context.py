...
def _resolve_trellis_root(hook_input: dict[str, Any]) -> Path | None:
    ...
    ticket, and `task.py start` degrades. The hook's own cwd is reliable where
    the payload is not: hosts launch it from the project root, which is why a
    relative `command` in settings.json resolves at all. Try the payload first
    (it is right on hosts that set it), then fall back.
    """
