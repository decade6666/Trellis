...
describe("assembleContext with trusted roots (#414 repro)", () => {
  ...
  it.skipIf(isWin)(
    "does not treat filesystem root as a universal trust root (startsWith + sep)",
    () => {
      // isUnderRoot(real, "/") is real === "/" || real.startsWith("//").
      // Allowlisting "/" therefore must NOT open every absolute path — the
      // historical path.relative jail would, which is why OMP must mirror
      // this startsWith form instead.
      const outside = path.join(tmpDir, "outside-root-trust");
      fs.mkdirSync(outside, { recursive: true });
      const secret = path.join(outside, "secret.md");
      fs.writeFileSync(secret, "should not load via root allowlist\n");
      fs.writeFileSync(
        path.join(cwd, ".trellis", "config.yaml"),
        "channel:\n  trusted_context_dirs:\n    - /\n",
      );
      ...
      const trustedRoots = resolveTrustedRoots(cwd);
      expect(trustedRoots).toEqual(["/"]);
      const result = assembleContext(cwd, [secret], [], trustedRoots);
      expect(result.prompt).toBe("");
    },
  );
