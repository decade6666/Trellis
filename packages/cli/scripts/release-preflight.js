 ...
 *   1. Version match between `@decade6666/trellis` and
 *      `@decade6666/trellis-core` (and the current git tag when checked from
 *      a tag context).
 ...
 *                                    on @decade6666/trellis-core resolves
...
function verifyPackedCli() {
  ...
  try {
    ...
    const dep = packedPkg.dependencies?.["@decade6666/trellis-core"];
    if (!dep) {
      fail(`packed CLI is missing dependency on @decade6666/trellis-core.`);
    }
    if (dep !== v.cliVersion) {
      fail(
        `packed CLI depends on @decade6666/trellis-core@"${dep}" but expected exact "${v.cliVersion}".\n` +
          `pnpm should rewrite workspace:* to the exact published version; got "${dep}" instead.`,
      );
    }
    console.log(
      `${GREEN}ok${RESET} packed CLI pins @decade6666/trellis-core to exact ${v.cliVersion}.`,
    );
  } finally {
